...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                    ...
                    { display: '总金额', name: 'Amount', width: 70 },
                    { display: '关联人员', name: 'RelevantPerson', width: 70 },
                ...
                url: "../../data/Financial_Fixed_Cost.ashx?Action=grid&IsApr=" + getparastr("IsApr"),
                width: '100%',
                height: '100%',
                heightDiff: -1,
        ...
        function doserch() {
            ...
            var manager = $("#maingrid4").ligerGetGridManager();
            manager.GetDataByURL("../../data/Financial_Fixed_Cost.ashx?" + serchtxt);
      ...
      function f_reload() {
        ...
        function view() {
            ...
            if (row) {
                f_openWindow_view('CRM/finance/Financial_Fixed_Cost_add.aspx?fid=' + row.F_Num, "查看人工费用", 690, 500);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
        function add() {
            f_openWindow('CRM/finance/Financial_Fixed_Cost_add.aspx', "新建固定费用", 690, 300);
        }
        ...
        function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('CRM/finance/Financial_Fixed_Cost_add.aspx?fid=' + row.F_Num, "修改固定费用", 690, 300);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
        function tj() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow_sh('CRM/finance/Financial_Fixed_Cost_add.aspx?fid=' + row.F_Num, "审核变动费用", 690, 500);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
        function del() {
            ...
            if (row) {
                $.ligerDialog.confirm(" 删除后不能恢复，\n您确定要移除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../../data/Financial_Fixed_Cost.ashx", type: "POST",
