...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            if (getparastr("fid") || getparastr("id")) {
                loadorder(getparastr("fid"), getparastr("id"));
               
            }
            else {
                alert("系统无法在没有单据的情况下付款！");
        ...
        function loadorder(F_Num, id) {
      
            $.ajax({

                type: "GET",
                url: "../../data/Financial_Labour_Cost.ashx", /* 注意后面的名字对应CS的方法名称 */
                data: { Action: 'form', fid: F_Num, id: id ,rnd: Math.random() }, /* 注意参数的格式和名称 */
                ...
                success: function (result) {
                    ...
                    if (getparastr("sklb"))
                    {
                        if (getparastr("sklb") == "addDj")
                            $("#T_receive_direction").ligerGetComboBoxManager().selectValue(1);
            ...
            if (getparastr("customerid"))
             ...
             if ($(form1).valid()) {
                if (parseFloat($("#T_invoice_amount").val().replace(/\$|\,/g, '')) > parseFloat($("#T_arrears_amount").val().replace(/\$|\,/g, '')) && $("#T_receive_direction").val() == "收款") {
                        alert('您的付款金额已经大于未付金额！');
                        return;
                    }
                    if (parseFloat($("#T_invoice_amount").val().replace(/\$|\,/g, '')) > parseFloat($("#T_receive_amount").val().replace(/\$|\,/g, '')) && $("#T_receive_direction").val() == "退款") {
                        alert('您的退款金额已经大于已付金额！');
                        return;
                    }
        ...
        <div>
            <table style="width: 600px; margin: 5px;" class='bodytable1'>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <input type="text" id="T_Customer" name="T_Customer" ltype="text" ligerui="{width: 486,disabled:true}" style="width: 460px;" />
                        <input type="hidden" id="F_Num" name="F_Num" />
                       <input type="hidden" id="T_Customer_val" name="T_Customer_val" />  
                    </td>
                </tr>
                <tr>
                    <td width="75" align="right">
...
<%--                    <td align="right">
                ...
                <tr>
                    <td class="table_title1" colspan="4">付款信息</td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <div align="left" style="width: 60px">付款类别：</div>
                    </td>
                    <td>
                        <div style="width: 182px; float: left">
                            <input type="text" id="T_receive_direction" name="T_receive_direction" ltype="select" ligerui="{readonly:true,disabled:true,width:182,data:[{id:1,text:'付款'},{id:-1,text:'退款'},{id:2,text:'收定金'},{id:-2,text:'退定金'},{id:3,text:'收装修款'},{id:-3,text:'退装修款'},{id:4,text:'收其他费用'},{id:-4,text:'退其他费用'}]}" validate="{required:true}"  />
                        </div>
