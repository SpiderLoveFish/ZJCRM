    ...
    <script type="text/javascript">
        ...
        function f_openWindow_sync(url, title, width, height) {
            var dialogOptions = {
                width: width, height: height, title: title, url: url, buttons: [
                    {
                        text: '同步', onclick: function (item, dialog) {
        ...
        function view() {
            ...
            if (row) {
                f_openWindow_view('CRM/finance/Financial_Change_Cost_add.aspx?fid=' + row.F_Num, "查看人工费用", 690, 500);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
        function add() {
            f_openWindow('CRM/finance/Financial_Change_Cost_add.aspx', "新建变动费用", 690, 300);
        }

        function sync() {
            f_openWindow_sync('CRM/finance/SyncWX_Change_Cost.aspx', "同步微信变动费用", 500, 300);
        ...
        function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('CRM/finance/Financial_Change_Cost_add.aspx?fid=' + row.F_Num, "修改变动费用", 690, 300);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
        function tj() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow_sh('CRM/finance/Financial_Change_Cost_add.aspx?fid=' + row.F_Num, "审核变动费用", 690, 500);
            } else {
                $.ligerDialog.warn("请选择行");
            }
        }
