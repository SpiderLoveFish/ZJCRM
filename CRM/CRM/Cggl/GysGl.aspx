...
<head> 
    ...
    <script type="text/javascript">
        ...
        $(function () {
           // //debugger;
            ...
            $("#maingrid4").ligerGrid({
                ...
                url: "GysGl.aspx?cmd=grid&rnd=" + Math.random(),
                width: '100%',
                height: '100%',
                heightDiff: -1,
        ...
        function toolbar() {
            ////debugger;
            $.getJSON("../../data/toolbar.ashx?Action=GetSys&mid=144&rnd=" + Math.random(), function (data, textStatus) {
                var items = [];
                var arr = data.Items;
        ...
        function doserch() {
            ...
            manager.GetDataByURL("GysGl.aspx?" + serchtxt);
        }
        function add() {
            f_openWindow("CRM/Cggl/GysGl_Add.aspx", "新增供应商", 600, 330);
        }
        function edit() {
            var manager = $("#maingrid4").ligerGetGridManager();
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow("CRM/Cggl/GysGl_Add.aspx?cid=" + row.ID, "供应商维护", 600, 330);
            }
            else {
        ...
        function del() {
            ...
            if (row) {
                $.ligerDialog.confirm("确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "GysGl_Add.aspx?cmd=del&cid=" + row.ID + "&rnd=" + Math.random(), type: "POST",
