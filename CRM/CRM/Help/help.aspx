...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#tree1").ligerTree({
                url: '../../data/public_help_category.ashx?Action=tree&rnd=' + Math.random(),
                onSelect: onSelect,
                idFieldName: 'id',
            ...
            $("#maingrid4").ligerGrid({
                ...
                dataAction: 'server',
                url: "../../data/public_help.ashx?Action=grid&categoryid=0&rnd=" + Math.random(),
                pageSize: 30,
        ...
        function toolbar() {
            $.getJSON("../../data/toolbar.ashx?Action=GetSys&mid=98&rnd=" + Math.random(), function (data, textStatus) {
                //alert(data);
                var items = [];
                var arr = data.Items;
        ...
        function onSelect(note) {
            ...
            var url = "../../data/public_help.ashx?Action=grid&categoryid=" + note.data.id + "&rnd=" + Math.random();
        ...
        function doserch() {
            ...
            var manager = $("#maingrid4").ligerGetGridManager();
            manager.GetDataByURL("../../data/public_help.ashx?" + serchtxt);
        }
        ...
        function edit() {
            ...
            if (row)
                f_openWindow('crm/help/help_add.aspx?pid=' + row.id, "修改帮助", 780, 510);
            else
        ...
        function add() {
            ...
            if (notes != null && notes != undefined) {
                f_openWindow('crm/help/help_add.aspx?categoryid=' + notes.data.id, "新增帮助", 780, 510);
            }
            else {
                $.ligerDialog.warn('请选择帮助目录！');
            }
        }
        ...
        function del() {
            ...
            var row = manager.getSelectedRow();
            if (row){
                $.ligerDialog.confirm("帮助删除不能恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../../data/public_help.ashx", type: "POST",
        ...
        function view(id) {
            var dialogOptions = {
                width: 770, height: 510, title: "查看帮助", url: 'crm/help/help_view.aspx?pid=' + id + '&rnd=' + Math.random(), buttons: [
                        {
                            text: '关闭', onclick: function (item, dialog) {
                                dialog.close();
                            }
