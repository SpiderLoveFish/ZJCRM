<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Jifen_yg.aspx.cs" Inherits="Jifen.Jifen_yg" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                ...
                onSelectRow: function (data, rowindex, rowobj) {
                    ...
                    var url = "Jifen_yg.aspx?cmd=grid1&rid=" + data.ID + "&rnd=" + Math.random();
                    manager.GetDataByURL(url);
                },
                dataAction: 'server', pageSize: 30, pageSizeOptions: [20, 30, 50, 100],
                url: "Jifen_yg.aspx?cmd=grid&rnd=" + Math.random(),
                width: '100%',
                height: '65%',
                heightDiff: -1,
        ...
        function dosearch() {
            ...
            manager.GetDataByURL("Jifen_yg.aspx?" + serchtxt);
        ...
        function add() {
            ...
            if (row) {
                f_openWindow("CRM/Jifen/Jifen_yg_add.aspx?cid=" + row.ID + "&jid=0&zjf=" + row.Jf + "&jlx=0", "发放积分", 580, 400);
            } else {
        ...
        function edit() {
            ...
            if (row && row1) {
                if (row1.Jflx == "0")
                    f_openWindow("CRM/Jifen/Jifen_yg_add.aspx?cid=" + row.ID + "&jid=" + row1.JfID + "&zjf=" + row.Jf + "&jlx=0", "发放积分维护", 770, 530);
                else
                    f_openWindow("CRM/Jifen/Jifen_yg_add.aspx?cid=" + row.ID + "&jid=" + row1.JfID + "&zjf=" + row.Jf + "&jlx=1", "使用积分维护", 770, 530);
            }
            else {
                $.ligerDialog.warn('操作失败，请选择行！');
            }
        ...
        function del() {
            ...
            if (row) {
                $.ligerDialog.confirm("删除后将无法恢复，是否确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "Jifen_yg_add.aspx?cmd=del&jid=" + row.JfID + "&rnd=" + Math.random(), type: "POST",
        ...
        function f_save(item, dialog) {
            ...
            if (issave) {
                ...
                $.ajax({
                    url: "Jifen_yg_add.aspx", type: "POST",
                    data: issave,
                    beforesend: function () {
    ...
    <div class="az">
        <form id='serchform'>
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width: 62px;'>
                        <div style='width: 60px; text-align: right; float: right'>员工姓名：</div>
                    </td>
