<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Jcb_Ckgl.aspx.cs" Inherits="Jcb_Ckgl.Jcb_Ckgl" %>
...
<head> 
    ...
    <script type="text/javascript">
        ...
        $(function () {
           // //debugger;
            ...
            $("#maingrid4").ligerGrid({
                ...
                url: "Jcb_Ckgl.aspx?cmd=grid&rnd=" + Math.random(),
                width: '100%',
                height: '100%',
                heightDiff: -1,
        ...
        function toolbar() {
            ////debugger;
            $.getJSON("../../data/toolbar.ashx?Action=GetSys&mid=147&rnd=" + Math.random(), function (data, textStatus) {
                var items = [];
                var arr = data.Items;
        ...
        function doserch() {
            ...
            manager.GetDataByURL("Jcb_Ckgl.aspx?" + serchtxt);
        }
        function add() {
            f_openWindow("CRM/KcGl/Jcb_Ckgl_Add.aspx", "新增仓库", 600, 220);
        }
        function edit() {
            var manager = $("#maingrid4").ligerGetGridManager();
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow("CRM/KcGl/Jcb_Ckgl_Add.aspx?cid=" + row.ID, "仓库维护", 600, 220);
            }
            else {
        ...
        function del() {
            ...
            if (row) {
                $.ligerDialog.confirm("确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "Jcb_Ckgl_Add.aspx?cmd=del&cid=" + row.ID + "&rnd=" + Math.random(), type: "POST",
        ...
        function f_save(item, dialog) {
            ...
            if (issave) {
                ...
                $.ajax({
                    url: "Jcb_Ckgl_Add.aspx", type: "POST",
                    data: issave,
                    beforesend: function () {
