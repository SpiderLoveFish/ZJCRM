    ...
    <script type="text/javascript">
        ...
        function f_selectProductOK(item, dialog)
        {
            ...
            if (!dialog.frame.f_select()) {
                ...
                var url = '../../data/Purchase_CGLY.ashx?Action=savedetail&isauto=' + $("#T_companyid").val()+'&pid=' + $("#T_Pid").val() + "&bjlist=" + pidlist + '&rdm=' + Math.random();
                dosave(url,dialog);
            }
        }
        ...
        function loadGrid()
        {
                    ...
                    g = $("#maingrid4").ligerGrid({
                        ...
                        dataAction: 'server',
                        url: "../../data/Purchase_CGLY.ashx?Action=griddetail&pid=" + getparastr("pid") + "&rnd=" + Math.random(),
                        pageSize: 30,
        ...
        function updateprice(pid,price)
        {
            $.ajax({
                type: 'post',
                url: "../../data/Purchase_CGLY.ashx?Action=updateprice&pid=" +pid+ "&price="+price+"&rdm=" + Math.random(),
                success: function (data) {
                    if (data == 'false') {
        ...
        function submit111(status,purid,pid,cid) {
            $.ajax({
                type: 'post',
                url: "../../data/Purchase_CGLY.ashx?Action=updatesfky&pid=" +pid + "&cid=" + cid+ "&status=" + status + '&purid=' + purid + '&rdm=' + Math.random(),
                success: function (data) {
                    if (data == 'false') {
                        getmaxid();
                        $.ligerDialog.error("保存错误！！！重新保存！");
        ...
        function loadForm(oaid) {
             $.ajax({
                type: "GET",
                url: "../../data/Purchase_CGLY.ashx", /* 注意后面的名字对应CS的方法名称 */
        ...
        function f_selectContact() {
            top.$.ligerDialog.open({
                zindex: 9003,
                title: '选择部门', width: 850, height: 400,
               
                //url: " hr/Getemp_Auth.aspx?auth=1", buttons: [
                url: "hr/hr_getpost.aspx", buttons: [
        ...
        function f_selectContactOK(item, dialog) {
          ...
          // alert( JSON.stringify(data));
            fillemp(data.dep_id, data.depname );
        ...
        function fillemp(id,  name ) {
            $("#T_companyid").val(id);
            $("#T_companyname").val(name); 
        }
        ...
        function addpur()
        {
            ...
            $.ajax({
                type: 'post',
                url: "../../data/Purchase_CGLY.ashx?Action=savetemp&pid=" + $("#T_Pid").val() + "&cid=" + $("#T_companyid").val() + "&remark=" + $("#T_remarks").val() + '&supid=' + $("#T_gysid").val() + '&isgd=' + isgd + '&rdm=' + Math.random(),
                success: function (data) {
                    if (data == 'false') {
                        getmaxid();
                        $.ligerDialog.error("保存错误！！！重新保存！");
        ...
        function doserch() {
            var manager = $("#maingrid4").ligerGetGridManager();
            manager.GetDataByURL("../../data/Purchase_CGLY.ashx?Action=griddetail&pid=" + $("#T_Pid").val());
        }
        function fload()
        {
...
<body style="padding: 0px;overflow:hidden;">
    <form id="form1" onsubmit="return false">
        ...
        <table style="width: 550px; margin: 5px;" class='bodytable1'>
            ...
            <tr>
                <td>
                    <div style="width: 70px; text-align: right; float: right">部门信息：</div>
                </td>
                <td  >
             ...
             <td   ><span style="width: 60px; text-align: right; float: right">施工监理：</span></td>
               ...
               <tr>
                 ...
                 <td colspan="2"  ><input id="T_remarks" name="T_remarks" type="text" ltype="text"  ligerui="{width:300}" /></td>

                   <td> 
                  <td    ><div style="width: 70px; text-align: right; float: right">采购类型：</div></td>
                 <td    ><input type="text" id="T_birthday_lunar" name="T_birthday_lunar" style="width: 66px" 
                                ltype="select" ligerui="{width:66,data:[{id:'1',text:'办公用品'},{id:'2',text:'其他'}]}"  /></td>
