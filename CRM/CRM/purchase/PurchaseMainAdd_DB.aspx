...
<head>
    ...
    <script type="text/javascript">
            ...
            if(price>0||editsum>0||Remarks.length>0)
            {
                ...
                if (row) {
                    $.ajax({
                        ...
                        data: { Action: "saveupdatedetaildb", pid: $("#T_Pid").val(), mid: row.material_id, editsum: editsum, price: price, remaks: Remarks, customid: row.Customer_id, rnd: Math.random() },
        ...
        function toolbar() {
            ...
            //items.push({ type: 'button', text: '选择材料', icon: '../../images/search.gif', disable: true, click: function () { selall() } });
        ...
        function selall() {
            top.$.ligerDialog.open({
                ...
                url: "CRM/Purchase/SelectProduct_DB.aspx?ostyle=purchase&cid=" + $("#T_companyid").val()+"&style=all&pid=" + $("#T_Pid").val(), buttons: [
        ...
        function selgc() {
            top.$.ligerDialog.open({
                zindex: 9003,
                title: '选择全部材料', width: 850, height: 400,
                url: "CRM/Purchase/SelectProduct_DB.aspx?ostyle=purchasecid=" + $("#T_companyid").val() + "&style=all&pid=" + $("#T_Pid").val(), buttons: [
                    { text: '确定(F2)', onclick: f_selectProductOK },
                    { text: '取消', onclick: f_selectContactCancel }
                ]
            });
            ...
            //    url: "CRM/Purchase/SelectProduct_DB.aspx?style=cl&cid=" + $("#T_companyid").val() + "&pid=" + $("#T_Pid").val(), buttons: [
            //      { text: '确定(F2)', onclick: f_selectProductOK },
            //      { text: '取消', onclick: f_selectContactCancel }
            //    ]
            //});
        ...
        function f_selectProductOK(item, dialog)
        {
            ...
            if (!dialog.frame.f_select()) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    pidlist = pidlist + ',' + rows[i].material_id;

                }
                var url = '../../data/Purchase.ashx?Action=savedetaildb&cid=' + $("#T_companyid").val()+'&pid=' + $("#T_Pid").val() + "&bjlist=" + pidlist + '&rdm=' + Math.random();
                dosave(url,dialog);
            }
        }
        ...
        function loadGrid()
        {
                    ...
                    g = $("#maingrid4").ligerGrid({
                        columns: [
                            ...
                            { display: '型号', name: 'model', width: 120 },
                             {
                             ...
                             },
                                {
                                 ...
                                 {
                                     display: '可用数量', name: 'subtotal', type: 'float', width: 60, align: 'right',
                                     totalSummary:
                                     {
                                         type: 'sum'
                                     }
                            ...
                            {
                                display: '工地', name: 'customer', align: 'left', width: 150, type: 'text'
                            },   
                                {
        ...
        function loadForm(oaid) {
             $.ajax({
                ...
                data: { Action: 'formdb', pid: oaid, rnd: Math.random() }, /* 注意参数的格式和名称 */
        ...
        function f_selectContact() {
            if (sfcombox) {//确定后不能再选择
                return;
            }
            top.$.ligerDialog.open({
                zindex: 9003,
                title: '选择工地', width: 850, height: 400,
               
                //url: " hr/Getemp_Auth.aspx?auth=1", buttons: [
                url: "CRM/purchase/SelectCustomer.aspx?mugd=N", buttons: [
        ...
        function f_selectContactgys() {
          ...
          //  gcombkh.setEnabled(false);
            top.$.ligerDialog.open({
                zindex: 9003,
                title: '选择目标工地', width: 850, height: 400,

                //url: " hr/Getemp_Auth.aspx?auth=1", buttons: [
                url: "CRM/purchase/SelectCustomer.aspx?mugd=Y", buttons: [
        ...
        function f_selectContactgysOK(item, dialog) {
         ...
         //   fillempgys(data.ID, data.Name);
            fillempgys(data.CustomerID, data.CustomerName,
                data.sgjl, data.address);
            dialog.close();
        }
        ...
        function getmaxid() {
            ...
            $.ajax({
                type: "get",
                url: "../../data/Purchase.ashx", /* 注意后面的名字对应CS的方法名称 */
                data: { Action: 'getmaxdbid', rnd: Math.random() }, /* 注意参数的格式和名称 */
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (result) {
        ...
        function fillempgys(id, emp, sgjl, address) {
            if (id == $("#T_companyid").val())
            {
                alert("新工地和原工地不能相同！");
                return;
            }
            $("#T_gysid").val(id);
            $("#T_gysname").val(emp + '(' + address + ')');
            //$("#T_employee").val(sgjl);
            //$("#T_gysid").val(id);
            //$("#T_gysname").val(emp);
            getmaxid();
        ...
        function addpur()
        {
            ...
            $.ajax({
                type: 'post',
                url: "../../data/Purchase.ashx?Action=savetempdb&pid=" + $("#T_Pid").val() + "&cid=" + $("#T_companyid").val() + "&remark=" + $("#T_remarks").val() + '&supid=' + $("#T_gysid").val() + '&isgd=' + isgd + '&rdm=' + Math.random(),
                success: function (data) {
                    if (data == 'false') {
                        getmaxid();
                        $.ligerDialog.error("保存错误！！！重新保存！");
...
<body style="padding: 0px;overflow:hidden;">
    <form id="form1" onsubmit="return false">
        ...
        <table style="width: 550px; margin: 5px;" class='bodytable1'>
            ...
            <tr>
                <td>
                    <div style="width: 70px; text-align: right; float: right">选择工地：</div>
                </td>
                <td  >
                ...
                <td>
                    <div style="width: 70px; text-align: right; float: right">目标工地：</div>
                </td>
                <td  >
             ...
             <td   ><span style="width: 60px; text-align: right; float: right">施工监理：</span></td>
              ...
              <div style="width: 70px; text-align: right; float: right">调拨单号：</div></td>
