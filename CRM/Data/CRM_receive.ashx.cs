    ...
    public class CRM_receive : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "save_khjdgl")
            {
                ...
                //    model.Customer_name = PageValidate.InputText(dsorder.Tables[0].Rows[0]["Customer_name"].ToString(), 255);
                //}
                model.Customer_id = int.Parse(request["Customer_id"].ToString());  
                model.Customer_name = PageValidate.InputText(request["Customer_name"].ToString(), 255);
                //model.C_depid = int.Parse(request["T_dep_val"].ToString());
                //model.C_depname = PageValidate.InputText(request["T_dep"].ToString(), 255);
                //model.C_empid = int.Parse(request["T_employee_val"].ToString());
                //model.C_empname = PageValidate.InputText(request["T_employee1"].ToString(), 255);
                ...
                model.receive_direction_name = PageValidate.InputText(request["T_receive_direction"], 255);
                int i = 1; //1表示收款，-1表示退款
                if (model.receive_direction_id == -2 || model.receive_direction_id == -3)
                    i = -1;
                model.Receive_amount = i * model.receive_real;
            ...
            if (request["Action"] == "grid_khjdgl")
            {
                ...
                //权限
                DataSet ds = cci.GetList_khjdgl(PageSize, PageIndex, serchtxt, sorttext, out Total);

                string dt = Common.GetGridJSON.DataTableToJSON1(ds.Tables[0], Total);
                context.Response.Write(dt);
            }
            if (request["Action"] == "grid_enter")
            {
                int PageIndex = int.Parse(request["page"] == null ? "1" : request["page"]);
                int PageSize = int.Parse(request["pagesize"] == null ? "300" : request["pagesize"]);
                string sortname = request["sortname"];
                string sortorder = request["sortorder"];
                ...
                if (!string.IsNullOrEmpty(request["company"]))
                    serchtxt += " and B.Customer  like N'%" + PageValidate.InputText(request["company"], 250) + "%'";

                if (!string.IsNullOrEmpty(request["receive_num"]))
                    serchtxt += " and Receive_num+ A.remarks like N'%" + PageValidate.InputText(request["receive_num"], 50) + "%'";
                ...
                DataSet ds = cci.GetList_Enter(PageSize, PageIndex, serchtxt, sorttext, out Total);

                string dt = Common.GetGridJSON.DataTableToJSON1(ds.Tables[0], Total);
                context.Response.Write(dt);
            }
            if (request["Action"] == "updateisdelete")
            {
                //参数安全过滤
                string id = PageValidate.InputText(request["id"], 50);
                if(cci.UpdateIsDelete(id))
            ...
            if (request["Action"] == "updateisdelete_del")
            {
                //参数安全过滤
                string id = PageValidate.InputText(request["id"], 50);
                if (cci.UpdateIsDelete_del(id))
                    context.Response.Write("true");
                else context.Response.Write("false");
