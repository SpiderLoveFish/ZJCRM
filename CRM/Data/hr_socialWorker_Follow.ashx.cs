...
{
    /// <summary>
    /// hr_socialWorker_Follow 的摘要说明
    /// </summary>
    public class hr_socialWorker_Follow : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "save")
            {
                ...
                BLL.hr_socialWorker ccc = new BLL.hr_socialWorker();
                string cname = ccc.GetList("id=" + model.Customer_id).Tables[0].Rows[0]["name"].ToString();
                ...
                if (!string.IsNullOrEmpty(request["T_starttime1"]) && !string.IsNullOrEmpty(request["T_endtime1"]) && !string.IsNullOrEmpty(request["T_content1"]))
                {
                    model.StartTime = DateTime.Parse(request["T_starttime1"]).AddHours(zonediff);
                    model.EndTime = DateTime.Parse(request["T_endtime1"]).AddHours(zonediff);
                    model.Subject = PageValidate.InputText(request["T_content1"], 4000);
                }
                ...
                if (!string.IsNullOrEmpty(request["T_content"]))
                {
                    ...
                    modelcalendar.Category = "195";//其他人员跟进提醒
