...
{
    /// <summary>
    /// public_data_category 的摘要说明
    /// </summary>
    public class public_data_category : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            BLL.public_data_category ccpc = new BLL.public_data_category();
            Model.public_data_category model = new Model.public_data_category();
            ...
            if (request["Action"] == "save")
            {
                ...
                model.category = Common.PageValidate.InputText(request["T_category_name"], 250);
                model.icon = Common.PageValidate.InputText(request["T_category_icon"], 250);
                ...
                if (!string.IsNullOrEmpty(id) && id != "null")
                {
                    ...
                    else
                    {
                        ...
                        string EventTitle = model.category;
                        string EventType = "资料类别修改";
                        int EventID = model.id;
                        if (dr["category"].ToString() != request["T_category_name"])
                        {
                            log.Add_log(UserID, UserName, IPStreet, EventTitle, EventType, EventID, "资料类别", dr["category"].ToString(), request["T_category_name"]);
                        }
                        if (dr["icon"].ToString() != request["T_category_icon"])
                        {
                            log.Add_log(UserID, UserName, IPStreet, EventTitle, EventType, EventID, "类别图标", dr["icon"].ToString(), request["T_category_icon"]);
                        }
            ...
            if (request["Action"] == "grid")
            {
                ...
                if (!string.IsNullOrEmpty(request["company"]))
                    serchtxt += " and category like N'%" + PageValidate.InputText(request["company"], 50) + "%'";
            ...
            {
                DataSet ds = ccpc.GetList("parentid=0");
                StringBuilder str = new StringBuilder();
                str.Append("[");
            ...
            if (request["Action"] == "del")
            {
                ...
                BLL.public_data product = new BLL.public_data();
                ...
                else
                {
                    ...
                    {
                        //日志
                        string EventType = "资料类别删除";

                        int UserID = emp_id;
                        string UserName = empname;
                        string IPStreet = request.UserHostAddress;
                        int EventID = int.Parse(c_id);
                        string EventTitle = ds.Tables[0].Rows[0]["category"].ToString();
