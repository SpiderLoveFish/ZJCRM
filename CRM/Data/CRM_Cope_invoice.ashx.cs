...
{
    /// <summary>
    /// CRM_Cope_invoice 的摘要说明
    /// </summary>
    public class CRM_Cope_invoice : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            BLL.CRM_Cope_invoice cci = new BLL.CRM_Cope_invoice();
            Model.CRM_Cope_invoice model = new Model.CRM_Cope_invoice();
            ...
            if (request["Action"] == "save")
            {
                ...
                BLL.Purchase_Main order = new BLL.Purchase_Main();
                DataSet dsorder = order.GetList_main("Purid='" + orderid + "'");

                model.order_id =  orderid ;
                if (dsorder.Tables[0].Rows.Count > 0)
                {
                    model.Customer_id = int.Parse(dsorder.Tables[0].Rows[0]["customid"].ToString());
                    model.Customer_name = PageValidate.InputText(dsorder.Tables[0].Rows[0]["Customer"].ToString(), 255);
                }
