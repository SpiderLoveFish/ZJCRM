...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#tree1").ligerTree({
                url: "../data/smsmodel.ashx?Action=tree&parentid=23&rnd=" + Math.random(),
                onSelect: onSelect,
                idFieldName: 'id',
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                    ...
                    { display: '标题', name: 'NoticeTypeName', width: 150 },
                 {
                     display: '内容', name: 'NoticeContent', align: 'left', width: 250, render: function (item) {
                ...
                url: "../../data/smsmodel.ashx?Action=grid_alarm&params_id=0&rnd=" + Math.random(),
                pageSize: 30,
        ...
        function onSelect(note) {
            ...
            var url = "../../data/smsmodel.ashx?Action=grid_alarm&params_id=" + note.data.id + "&rnd=" + Math.random();
            manager.GetDataByURL(url);
            checkedID = [];
        }
        //查询
        function doserch() {
            var sendtxt = "&Action=grid_alarm&rnd=" + Math.random();
        ...
        function f_openWindow(url, title, width, height) {
            var dialogOptions = {
                width: width, height: height, title: title, url: url, buttons: [
                    {
                        text: '语音播报', onclick: function (item, dialog) {
                            f_save_yy(item, dialog);
                        }
                    },
                        {
        ...
        function add() {
            ...
            if (notes != null && notes != undefined) {
                f_openWindow('system/alarm_send.aspx?params_id=' + notes.data.id +"&name=" + notes.data.text, "新增模板", 650, 500);
            }
            else {
        ...
        //}
        function f_save_yy(item, dialog) {
            var issave = dialog.frame.f_save_yy();
            if (issave) {
