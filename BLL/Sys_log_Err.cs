...
namespace XHD.BLL
{
	...
	public partial class Sys_log_Err
	{
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.Sys_log_Err GetModelByCache(int id)
		{
			...
			return (XHD.Model.Sys_log_Err)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.Sys_log_Err> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.Sys_log_Err> DataTableToList(DataTable dt)
		{
			List<XHD.Model.Sys_log_Err> modelList = new List<XHD.Model.Sys_log_Err>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.Sys_log_Err model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.Sys_log_Err();
