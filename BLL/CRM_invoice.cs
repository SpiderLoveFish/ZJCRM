...
namespace XHD.BLL
{
	...
	{
		private readonly XHD.DAL.CRM_invoice dal=new XHD.DAL.CRM_invoice();
		public CRM_invoice()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.CRM_invoice GetModelByCache(int id)
		{
			...
			return (XHD.Model.CRM_invoice)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_invoice> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_invoice> DataTableToList(DataTable dt)
		{
			List<XHD.Model.CRM_invoice> modelList = new List<XHD.Model.CRM_invoice>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.CRM_invoice model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.CRM_invoice();
