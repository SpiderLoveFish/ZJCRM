...
namespace XHD.BLL
{
	...
	{
		private readonly XHD.DAL.CRM_order dal=new XHD.DAL.CRM_order();
		public CRM_order()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.CRM_order GetModelByCache(int id)
		{
			...
			return (XHD.Model.CRM_order)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_order> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_order> DataTableToList(DataTable dt)
		{
			List<XHD.Model.CRM_order> modelList = new List<XHD.Model.CRM_order>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.CRM_order model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.CRM_order();
