...
namespace XHD.BLL
{
	...
	{
		private readonly XHD.DAL.CRM_contract dal=new XHD.DAL.CRM_contract();
		public CRM_contract()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.CRM_contract GetModelByCache(int id)
		{
			...
			return (XHD.Model.CRM_contract)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_contract> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.CRM_contract> DataTableToList(DataTable dt)
		{
			List<XHD.Model.CRM_contract> modelList = new List<XHD.Model.CRM_contract>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.CRM_contract model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.CRM_contract();
