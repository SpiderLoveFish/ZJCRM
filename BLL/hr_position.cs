...
namespace XHD.BLL
{
	...
	{
		private readonly XHD.DAL.hr_position dal=new XHD.DAL.hr_position();
		public hr_position()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.hr_position GetModelByCache(int id)
		{
			...
			return (XHD.Model.hr_position)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.hr_position> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.hr_position> DataTableToList(DataTable dt)
		{
			List<XHD.Model.hr_position> modelList = new List<XHD.Model.hr_position>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.hr_position model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.hr_position();
