...
namespace XHD.BLL
{
	...
	public partial class Sys_App
	{
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.Sys_App GetModelByCache(int id)
		{
			...
			return (XHD.Model.Sys_App)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.Sys_App> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.Sys_App> DataTableToList(DataTable dt)
		{
			List<XHD.Model.Sys_App> modelList = new List<XHD.Model.Sys_App>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.Sys_App model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new XHD.Model.Sys_App();
