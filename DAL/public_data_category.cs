...
namespace XHD.DAL
{
	...
	public partial class public_data_category
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "public_data_category"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from public_data_category");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(XHD.Model.public_data_category model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into public_data_category(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XHD.Model.public_data_category model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update public_data_category set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from public_data_category ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
		...
		/// 得到一个对象实体
		/// </summary>
		public XHD.Model.public_data_category GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,category,parentid,icon,isDelete,Delete_id,Delete_time,orderid from public_data_category ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
			...
			XHD.Model.public_data_category model=new XHD.Model.public_data_category();
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM public_data_category ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
		{
			...
			strSql.Append("select ");
			strSql.Append(" top " + PageSize + " * FROM public_data_category ");
			strSql.Append(" WHERE id not in ( SELECT top " + (PageIndex - 1) * PageSize + " id FROM public_data_category ");
			strSql.Append(" where " + strWhere + " order by " + filedOrder + " ) ");
			strSql1.Append(" select count(id) FROM public_data_category ");
			if (strWhere.Trim() != "")
			{
