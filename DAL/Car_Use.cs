...
namespace XHD.DAL
{
	...
	public partial class Car_Use
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "Car_Use"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from Car_Use");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(XHD.Model.Car_Use model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Car_Use(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XHD.Model.Car_Use model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Car_Use set ");
		...
		/// 删除一条数据
		/// </summary>
		public bool Delete(int id,int carid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Car_Use ");
			strSql.Append(" where id="+ id + "");
		...
		/// 得到一个对象实体
		/// </summary>
		public XHD.Model.Car_Use GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,carid,carNumber,userPerson,useBeginTiime,useEndTiime,Remarks,IsStatus from Car_Use ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			XHD.Model.Car_Use model=new XHD.Model.Car_Use();
		...
		/// 得到一个对象实体
		/// </summary>
		public XHD.Model.Car_Use DataRowToModel(DataRow row)
		{
			XHD.Model.Car_Use model=new XHD.Model.Car_Use();
			if (row != null)
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" id,carid,carNumber,userPerson,useBeginTiime,useEndTiime,Remarks,IsStatus,savetime,gtime ");
			strSql.Append(" FROM Car_Use ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Car_Use T ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("UPDATE	 Car_Use SET IsStatus= CASE IsStatus WHEN 'Y' THEN 'N' WHEN 'N' THEN 'Y' ELSE 'Y' END ");
            if (sfsh != "Y")//是否审核，归还的时候才更新时间
                strSql.Append(" ,gtime=getdate() ");
        ...
        public DataSet GetList(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            strSql.Append("select ");
            strSql.Append(" top " + PageSize + " * FROM Car_Use ");
            strSql.Append(" WHERE id not in ( SELECT top " + (PageIndex - 1) * PageSize + " id FROM Car_Use ");
            strSql.Append(" where " + strWhere + " order by " + filedOrder + " ) ");
            strSql1.Append(" select count(id) FROM Car_Use ");
            if (strWhere.Trim() != "")
            {
