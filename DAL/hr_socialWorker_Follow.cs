...
{
	/// <summary>
	/// 数据访问类:hr_socialWorker_Follow
	/// </summary>
	public partial class hr_socialWorker_Follow
	{
		public hr_socialWorker_Follow()
		{}
		#region  Method
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "hr_socialWorker_Follow"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from hr_socialWorker_Follow");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(XHD.Model.hr_socialWorker_Follow model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into hr_socialWorker_Follow(");
            strSql.Append("Customer_id,Customer_name,Follow,Follow_date,Follow_Type_id,Follow_Type,department_id,department_name,employee_id,employee_name,isDelete,Delete_time,Subject,StartTime,EndTime)");
            ...
            parameters[12].Value = model.Subject;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XHD.Model.hr_socialWorker_Follow model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update hr_socialWorker_Follow set ");			
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from hr_socialWorker_Follow ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
		...
		/// 得到一个对象实体
		/// </summary>
		public XHD.Model.hr_socialWorker_Follow GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,Customer_id,Customer_name,Follow,Follow_date,Follow_Type_id,Follow_Type,department_id,department_name,employee_id,employee_name,isDelete,Delete_time,Subject,StartTime,EndTime from hr_socialWorker_Follow ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
			...
			XHD.Model.hr_socialWorker_Follow model=new XHD.Model.hr_socialWorker_Follow();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                {
                    model.Delete_time = DateTime.Parse(ds.Tables[0].Rows[0]["Subject"].ToString());
                }
                if (ds.Tables[0].Rows[0]["StartTime"] != null && ds.Tables[0].Rows[0]["StartTime"].ToString() != "")
                {
                    model.Delete_time = DateTime.Parse(ds.Tables[0].Rows[0]["StartTime"].ToString());
                }
                if (ds.Tables[0].Rows[0]["EndTime"] != null && ds.Tables[0].Rows[0]["EndTime"].ToString() != "")
                {
                    model.Delete_time = DateTime.Parse(ds.Tables[0].Rows[0]["EndTime"].ToString());
                }
				return model;
			}
			else
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select id,Customer_id,Customer_name,Follow,Follow_date,Follow_Type_id,Follow_Type,department_id,department_name,employee_id,employee_name,isDelete,Delete_time,Subject,StartTime,EndTime ");
            strSql.Append(" FROM hr_socialWorker_Follow ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" id,Customer_id,Customer_name,Follow,Follow_date,Follow_Type_id,Follow_Type,department_id,department_name,employee_id,employee_name,isDelete,Delete_time,Subject,StartTime,EndTime ");
            strSql.Append(" FROM hr_socialWorker_Follow   ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetList(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
		{
			...
			strSql.Append("select ");
            strSql.Append(" top " + PageSize + " * FROM hr_socialWorker_Follow a LEFT JOIN (SELECT id AS typeid,setcolor FROM  Param_SysParam) b ON  b.typeid=a.Follow_Type_id ");
			strSql.Append(" WHERE id not in ( SELECT top " + (PageIndex - 1) * PageSize + " id FROM hr_socialWorker_Follow ");
			strSql.Append(" where " + strWhere + " order by " + filedOrder + " ) ");
            strSql1.Append(" select count(id) FROM hr_socialWorker_Follow ");
			if (strWhere.Trim() != "")
			{
        ...
        public DataSet Reports_year(string items, int year, string where)
        {
            ...
            strSql.Append("    from dbo.hr_socialWorker_Follow ");
        ...
        public DataSet Compared_follow(string year1, string month1, string year2, string month2)
        {
            ...
            strSql.Append(" FROM hr_socialWorker_Follow group by Follow_Type");
        ...
        public DataSet Compared_empcusfollow(string year1, string month1, string year2, string month2, string idlist)
        {
            ...
            strSql.Append(" SUM(case when YEAR( hr_socialWorker_Follow.Follow_date)=('" + year1 + "') and MONTH(hr_socialWorker_Follow.Follow_date)=('" + month1 + "') then 1 else 0 end) as dt1, ");
            strSql.Append(" SUM(case when YEAR( hr_socialWorker_Follow.Follow_date)=('" + year2 + "') and MONTH(hr_socialWorker_Follow.Follow_date)=('" + month2 + "') then 1 else 0 end) as dt2 ");
            strSql.Append(" from hr_employee left outer join hr_socialWorker_Follow ");
            strSql.Append(" on hr_employee.ID=hr_socialWorker_Follow.employee_id ");
