...
namespace XHD.DAL
{
	...
	public partial class Budge_BasicMain
	{
        ...
        public DataSet GetBudge_BasicMain(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            strSql1.Append(" select count(A.id) FROM Budge_BasicMain A");
            strSql1.Append("  LEFT JOIN dbo.CRM_Customer B ON A.customer_id=B.id");
        ...
        public DataSet GetPrintCount(string bid)
        {
            ...
            sb.AppendLine("  SELECT    c.c_style ,");
            sb.AppendLine("            ISNULL(SUM(SubTotal), 0) AS je ,");
            sb.AppendLine("            bp.id AS ComponentID ,");
            ...
            sb.AppendLine("            LEFT JOIN dbo.Budge_BasicPart bp ON bp.BP_Name = ComponentName");
            sb.AppendLine("  WHERE     budge_id = '"+bid+"'");
            sb.AppendLine("  GROUP BY  c.c_style ,");
            sb.AppendLine("            bp.id ,");
            sb.AppendLine("            ComponentName");
            sb.AppendLine("  SELECT    bp.id AS ComponentID ,");
            sb.AppendLine("            ComponentName ,");
            sb.AppendLine("            COUNT(xmid) AS zl ,");
            sb.AppendLine("            ISNULL(SUM([SUM]), 0) AS zsl ,");
            sb.AppendLine("            ISNULL(SUM(a.TotalPrice * [SUM]), 0) AS zje ,");
            sb.AppendLine("            ISNULL(SUM(a.zc_price * [SUM]), 0) AS zc_zje ,");
            sb.AppendLine("            ISNULL(SUM(a.fc_price * [SUM]), 0) AS fc_zje ,");
            sb.AppendLine("            ISNULL(SUM(a.rg_price * [SUM]), 0) AS rg_zje ,");
            sb.AppendLine("            ( SELECT    ISNULL(je, 0) AS je");
            sb.AppendLine("              FROM      #temp");
            sb.AppendLine("              WHERE     C_style = '主材'");
            sb.AppendLine("                        AND ComponentName = A.ComponentName");
            sb.AppendLine("            ) AS zcje ,");
            sb.AppendLine("            ( SELECT    ISNULL(je, 0) AS je");
            sb.AppendLine("              FROM      #temp");
            sb.AppendLine("              WHERE     C_style = '基建'");
            sb.AppendLine("                        AND ComponentName = A.ComponentName");
            sb.AppendLine("            ) jcje");
            sb.AppendLine("  FROM      dbo.Budge_BasicDetail A");
            sb.AppendLine("            LEFT JOIN dbo.CRM_product B ON A.xmid = B.product_id");
            ...
            sb.AppendLine("  SELECT    9999 AS ComponentID ,");
            sb.AppendLine("            '合计' ,");
            sb.AppendLine("            COUNT(xmid) AS zl ,");
            sb.AppendLine("            ISNULL(SUM([SUM]), 0) AS zsl ,");
            sb.AppendLine("            ISNULL(SUM(a.TotalPrice * [SUM]), 0) AS zje ,");
            ...
            sb.AppendLine("            ISNULL(SUM(a.fc_price * [SUM]), 0) AS fc_zje ,");
            sb.AppendLine("            ISNULL(SUM(a.rg_price * [SUM]), 0) AS rg_zje ,");
            sb.AppendLine("            0 AS zcje ,");
            sb.AppendLine("            0 AS jcje");
            sb.AppendLine("  FROM      dbo.Budge_BasicDetail A");
            sb.AppendLine("            LEFT JOIN dbo.CRM_product B ON A.xmid = B.product_id");
            sb.AppendLine("            LEFT JOIN dbo.CRM_product_category C ON B.category_id = C.id");
            sb.AppendLine("  WHERE     budge_id = '" + bid + "'");
            sb.AppendLine("  ORDER BY  ComponentID");
            sb.AppendLine("");
            sb.AppendLine("");
            return DbHelperSQL.Query(sb.ToString());
        }
